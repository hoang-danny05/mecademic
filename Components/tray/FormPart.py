...
class Component(Logger):
    ...
    def pressButton(self):
        ...
        self.rbt.MoveJoints(90,0,0,0,90,0) #home
        self.rbt.MoveLinRelWrf(0, 90, -66, 0, 0, 0) #down
        self.rbt.MoveLinRelWrf(70, 3, 0, 0, 0, 0) #move to first slot
        self.rbt.Delay(.5)

        #self.rbt.MoveJoints(56.4198, 28.02392, 39.36864, -35.7222, -71.32128, 12.97013)
        self._moveToIndex(self.index)
        self.rbt.MoveLinRelWrf(0, 0, -7, 0, 0, 0)
        self.rbt.Delay(.5)
        if (not self.debug):
    ...
    def flux(self):
        ...
        if (not self.debug):
            self.switch.assert_on()
        self.rbt.MoveJoints(35,0,0,0,35,90)
        self.rbt.SetJointVel(10)
        if (not self.debug):
            self.switch.assert_on()
        self.rbt.MoveJoints(35,9,0,0,30,90)
        self.rbt.MoveJoints(35,9,0,0,30,90) # before pulling out
        self.rbt.SetJointVel(50)
        self.rbt.SetCartLinVel(10)
        self.rbt.MoveLinRelWrf(0, 0, -12, 0, 0, 0) # goin down!! og: 12
    ...
    def solder(self):
        ...
        self.rbt.MoveJoints(0,0,0,0,75,-90)
        self.rbt.SetCartLinVel(20)
        # og 65
        try: 
            input("Did the part flux correctly? CTRL + C if NO, ENTER if YES")
            self.rbt.MoveLinRelWrf(65.5, 0, -3.6, 0, 0, 0) # moving to wave
            ...
            self.rbt.Delay(.6)
            self.rbt.SetCartLinVel(20)
            self.rbt.MoveLinRelWrf(-3, 0, 10, 0, 0, 0)
